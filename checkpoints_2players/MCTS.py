...
def initDistMatrix(N: int):
    D = np.zeros((N, N)).astype(float)
    for j in range(N):
        for i in range(N):
            D[j, i] = np.abs(j-i)
    return D
...
class MCTS():
    ...
    def getActionProb(self, canonicalBoard, temp=1, force_full_search=False, bias=None):
        ...
        kl = (np.log(probs+EPS) - np.log(self.nodes_data[s][2]+EPS)).dot(probs).item()

        M = initDistMatrix(len(probs))
        emd = ot.emd2(probs, self.nodes_data[s][2], M)
        surprise = emd

        print(f"EMD: {emd}, KL: {kl}, EMD-KL: {emd-kl}")
        ...
        if bias == "horizon": #水平戦略
            print("Probs: ", probs)
            nob = self.game.getNobleGemIDs(self.board)
            valids = self.game.getValidMoves(self.board, 0)
            for m in [m_ for m_, v in enumerate(valids) if v>0]:
                pass

            
        #elif bias == "vertical": #垂直戦略
