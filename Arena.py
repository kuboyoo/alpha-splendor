...
class Arena():
    ...
    def __init__(self, player1, player2, player3, game, args, display=None):
        """
        Input:
        ...
        model_name = "./splendor/pretrained_%dplayers.pt" % game.getNumberOfPlayers()
        nn_args = dict(lr=None, dropout=0., epochs=None, batch_size=None, nn_version=-1)
    ...
    def playGame(self, verbose=False, other_way=False):
        ...
        while not self.game.getGameEnded(board, curPlayer).any():
            ...
            if verbose:
                ...
                savePkl(self.record_dir.joinpath(f"board_turn_{it}.pkl"), board)
